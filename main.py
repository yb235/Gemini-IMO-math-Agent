...
# Following the paper specifications: Gemini 2.5 Pro with temperature 0.1
llm = ChatGoogleGenerativeAI(
    model="gemini-2.0-flash-exp",  # Using available Gemini 2.0 model (2.5 may not be released yet)
...
Before finalizing your output, carefully review your "Method Sketch" and "Detailed Solution" to ensure they are clean, rigorous, and strictly adhere to all instructions provided above. Verify that every statement contributes directly to the final, coherent mathematical argument."""),
    ...
    try:
        ...
        return {"error": "Failed to generate a solution."}
...
def corrector_node(state: GraphState):
    ...
    try:
        ...
        return {"error": "Failed to correct the solution."}
...
def generate_final_report(final_state, initial_state):
    ...
    if final_state.get("solution"):
        ...
        print("❌ No final solution was produced.")
    ...
    print(f"• Solution Accepted: {'✅ YES' if 'correct' in verification_status.lower() and verification_status.lower().startswith('the solution is correct') else '❌ NO'}")
    
    print("\n🏆 FINAL ANSWER:")
    print("=" * 60)
    if final_state.get("solution") and "correct" in verification_status.lower() and verification_status.lower().startswith("the solution is correct"):
        # Extract the final answer from the solution
        solution_text = final_state.get("solution", "")
        if "final answer" in solution_text.lower():
            ...
            print("✅ Problem solved - see detailed solution above")
    else:
        print("❌ No verified final answer available")
        print("🔄 Solution may require additional iterations or human intervention")
